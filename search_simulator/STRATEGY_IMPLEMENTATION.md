...
> 本文讨论工程实现难点，不重复 `ALGORITHM_DESIGN_V2.md` 中的数学设计；V1 只用于回看历史树搜索方案。
...
- 虚假预言家是否必须给出查验目标和结果尚待冻结，当前实现不得自行沿用旧的“禁止虚假查验”语义；
- 狼人可以把公开神职声明作为后续判断依据，但不能因此知道声明者的真实角色。

### 3.2 预言家不发言

- 预言家仍经过自己的发言位置，私有查验不进入公共信息；
- 空发言、拒绝声明和最短占位文本会形成不同的可观测历史，必须先冻结一种形式；
...
- 分别标记其处于发言顺序前四位或后三位；
- 虚假查验是否必需、允许哪些结果以及狼队是否协同声明必须由实验配置显式给出；
...
白天完整票型属于公开历史。只有当未来转移不再依赖每名玩家的具体投票时，才允许用票数向量或其他充分统计量做无损概率聚合；否则相同放逐结果下的不同公开票型仍是不同状态。“随机数投票”尚未冻结，不能用座位号或运行时单次随机抽取临时代替。
