...
发言阶段 action
  - `baseline`: 平铺、不点人、不跳身份
  - `silence`: 少说 / 过
  - `accusation`: 点某个存活他人
  - `support`: 挺某个存活他人
  - `vote_intent`: 表态要投某人
  - `seer_claim`: 只跳预言家，不带查验对象和结果(狼人可以编造假信息，预言家必须报真信息，其他职业只能跳身份)

`平民`：参与白天放逐投票。
  action_list:
    - 黑夜阶段：None
    - 发言阶段：[baseline, silence, accusation, support, vote_intent, seer_claim]

`狼人`：夜间知道其他狼人身份，可以私聊，投票选择一个要攻击的玩家。
  action_list:
    - 黑夜阶段：[刀人，自刀，跳过]
    - 发言阶段：[baseline, silence, accusation, support, vote_intent, seer_claim]

`预言家`：夜间查验一个其他玩家的身份（只知道好人/狼人）。
  action_list:
    - 黑夜阶段：[查验]
    - 发言阶段：[baseline, silence, accusation, support, vote_intent, seer_claim]

`女巫`：整局各有一瓶解药、一瓶毒药，每瓶只能用一次，不能在一夜同时使用。
  - 解药：夜里会被告知狼人刀了谁，可以选择救这个人。解药只能对当晚狼刀目标用，不能随便奶别人。仅第一夜能用解药自救，其后解药不能用于自救。
  - 毒药：指定一名除自己以外的玩家，天亮时无条件死亡。
  action_list:
    - 黑夜阶段：[使用解药，使用毒药，跳过]
    - 发言阶段：[baseline, silence, accusation, support, vote_intent, seer_claim]

`守卫`：无夜间信息。夜间选择一名除自己以外的玩家守卫，不能连续两夜守护同一名玩家。如果守卫对象是狼刀对象：若女巫没有使用解药救此对象，判定对象存活，狼刀失效；若女巫已对此使用解药，判定奶穿，对象死亡。守卫挡不住毒药。
  action_list:
    - 黑夜阶段：[守护]
    - 发言阶段：[baseline, silence, accusation, support, vote_intent, seer_claim]
...
目前提供了两种规则预设：
- `baseline`: 狼人夜间不能空刀、自刀；白天发言阶段只有预言家可以 `seer_claim`
- `cheap_talk`: 所有能力全开，狼人夜间可以空刀、自刀，白天可以悍跳编造查验信息；平民、女巫、守护可以跳预言家身份，不能编造信息

在 `.env` 中配置规则预设

```python
# Web / API 开局规则
# baseline：非预言家不能跳预言家；狼人不能空刀、不能自刀
# cheap_talk：全部能力开放（可跳预言家、可空刀、可自刀）
AIWEREWOLF_RULE_PRESET=cheap_talk
```
